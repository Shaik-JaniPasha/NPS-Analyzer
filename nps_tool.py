...
def process_nps(input_file):
    ...
    insights_list = [
        f"{round(negative/total*100,2)}% of feedback is negative, indicating key improvement areas",
        f"Top issue driving dissatisfaction: {top_issue}",
        f"{avoidable_pct}% of issues are avoidable and can be reduced with operational improvements"
    ]

    if positive > negative:
        insights_list.append("Overall sentiment is positive, but improvement opportunities exist")
    else:
        insights_list.append("Customer sentiment requires immediate attention")
